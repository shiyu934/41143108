...
在本次專案中實作一個以環狀鏈結串列搭配 Header Node 表示的多項式資料結構，並導入 Available-Space List 進行記憶體管理。

Available-Space List 的設計為此次最具挑戰性的環節。過去在 C++ 中習慣直接使用 new 與 delete，對於實作自訂節點池較為陌生。實際撰寫後，此機制避免了多次系統呼叫，並在多項式刪除時將原本 O(n) 的逐節點釋放，化為僅需調整指標的 O(1) 操作。是記憶體管理在系統程式中的設計是最有效率的方式處理。

環狀結串列提供了明確一致的邊界條件，以 ptr != head 為判斷基準才使資料走訪邏輯更為穩定。

這次實作驗證了課堂上學到的資料結構概念，更重要的是，在設計抽象資料型別時，外內部實作更細節的優化。
